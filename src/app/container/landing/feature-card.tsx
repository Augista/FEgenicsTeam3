...
function FeatureCard({ title, description, imageSrc, buttonText, className }: FeatureCardProps) {
  return (
    <div className={`flex w-4/5 justify-around items-center ${className}`}>
    <div className="bg-white w-fit p-8 rounded-full">
      <NextImage src={imageSrc} alt={title} width={180} height={180} className="rounded-full" />
      </div>
      <div className="flex items-start flex-col space-y-4 w-[30rem]">
        ...
        <Button size="small" href="">{buttonText}</Button>
      </div>
    </div>
  );
}
